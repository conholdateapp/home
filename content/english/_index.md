---
############################# Head ############################
head_title: "Free Online Document Viewer, Conversion, Signature, Editor & Annotation"
head_description: "View, convert, sign, edit, annotate, manage and share documents online for free"

############################### Banner ##############################
banner:
  enable: true
  bannerType: home # values: home, features
  title: "Cross Platform Automation Apps"
  subtitle: "Process PDF, Word, Excel, Powerpoint, OpenOffice and CAD files online "
  description: "Conholdate.app is an online document automation tool that provides various apps for document management and automation. Such as pptx merger, docx viewer, pdf to word converter and more "
  button:
    enable: true
    label: "Get Started"
    link: "/get-started"


############################# Products ############################
products:
  enable: true
  title: "Top Products"
  # products loop
  product:
    # product
    - image : "images/home-view.svg"
      title: "View"
      description: "View documents online. View over 90 document formats online with our build-in document viewer. No additional software installation required"
      link: "https://products.conholdate.app/viewer"
      linkText: "More"
    
    # product
    - image : "images/home-annotate.svg"
      title: "Annotate"
      description: "Annotate documents online. We support text, area, free hand polyline, distance and much more annotations"
      link: "https://products.conholdate.app/annotation"
      linkText: "More"

    # product
    - image : "images/home-sign.svg"
      title: "Sign"
      description: "Sign document online with an advanced electronic signature"
      link: "https://products.conholdate.app/signature"
      linkText: "More"

    # product
    - image : "images/home-edit.svg"
      title: "Edit"
      description: "Edit documents online. Our document editor engine supports all major file formats"
      link: "https://products.conholdate.app/editor"
      linkText: "More"

    # product
    - image : "images/home-convert.svg"
      title: "Convert"
      description: "Convert documents online. Our document conversion engine supports over 7000 conversion pairs"
      link: "https://products.conholdate.app/conversion"
      linkText: "More"
  singleLink: "https://products.conholdate.app"
  singleLinkText: "All Products"

############################# Stats ############################
stats:
  enable: true
  all: "PROCESSED DOCUMENTS"
  viewed: "VIEWED DOCUMENTS"
  converted: "CONVERTED DOCUMENTS"
  edited: "EDITED DOCUMENTS"
---